...
function processResult(result) {
    console.log(cache.get(result.imageId));
    if (cache.has(result.imageId)) {
        let recognizingImage = JSON.parse(cache.get(result.imageId));
        recognizingImage.objects[result.objectId].features.push(new FeatureDto('color', result.colors));
        ...
        let featureNamesQuery = "select feature.name from feature \n" +
            ...
            "where ot.name = '" + recognizingImage.objects[result.objectId].type + "'";
        let featureNames = sequelize.query(featureNamesQuery, {type: QueryTypes.SELECT}).then();
        if (featureNames.length === recognizingImage.objects[result.objectId].features.length) {
            ...
            if (location !== null) {
                ...
                setImageStatus(result.imageId, 'ready');
            } else {
                setImageStatus(result.imageId, 'error');
            }
        }
        cache.set(result.imageId, JSON.stringify(recognizingImage));
    }
}
...
amqplib.connect(amqpUrl, function (err, connection) {
    ...
    connection.createChannel(function (err, channel) {
        ...
        channel.assertQueue(AMQP_COLOR_QUEUE_OUT, {
            durable: false
        });
        console.log(" [*] Waiting for messages in %s. To exit press CTRL+C", AMQP_COLOR_QUEUE_OUT);
        channel.consume(AMQP_COLOR_QUEUE_OUT, function (msg) {
